...
export default async function FacultyMemberPage({ params }: PageProps) {
  ...
  const visibility = detailSettings?.visibility;
  const labelsConfig = detailSettings?.labels;
  const styling = detailSettings?.styling;

  // Labels with triple fallback (component > legacy > default)
  const labels = {
    backButtonText:
      labelsConfig?.backButtonText ?? detailSettings?.backButtonText ?? 'voltar para equipe',
    bioLabel: labelsConfig?.bioLabel ?? detailSettings?.bioLabel ?? '/// biografia',
    bioTitle: labelsConfig?.bioTitle ?? detailSettings?.bioTitle ?? 'Sobre',
    contactsLabel: labelsConfig?.contactsLabel ?? detailSettings?.contactsLabel ?? '/// contatos',
    contactsTitle: detailSettings?.contactsTitle || 'Informações de Contato',
    metricsLabel: labelsConfig?.metricsLabel ?? detailSettings?.metricsLabel ?? '/// métricas',
    ...
    educationLabel:
      labelsConfig?.educationLabel ?? detailSettings?.educationLabel ?? '/// formação',
    educationTitle:
      labelsConfig?.educationTitle ?? detailSettings?.educationTitle ?? 'Formação Acadêmica',
    researchLinesLabel:
      labelsConfig?.researchLinesLabel ?? detailSettings?.researchLinesLabel ?? '/// atuação',
    researchLinesTitle:
      labelsConfig?.researchLinesTitle ??
      detailSettings?.researchLinesTitle ??
      'Linhas de Pesquisa',
    projectsLabel: labelsConfig?.projectsLabel ?? detailSettings?.projectsLabel ?? '/// projetos',
    projectsTitle:
      labelsConfig?.projectsTitle ?? detailSettings?.projectsTitle ?? 'Projetos de Pesquisa',
    ...
    publicationsLabel:
      labelsConfig?.publicationsLabel ?? detailSettings?.publicationsLabel ?? '/// publicações',
    publicationsTitle:
      labelsConfig?.publicationsTitle ?? detailSettings?.publicationsTitle ?? 'Produção Científica',
    publicationsEmptyState:
      detailSettings?.publicationsEmptyState || 'Nenhuma publicação cadastrada.',
    adviseesLabel:
      labelsConfig?.adviseesLabel ?? detailSettings?.adviseesLabel ?? '/// orientações',
    adviseesTitle: labelsConfig?.adviseesTitle ?? detailSettings?.adviseesTitle ?? 'Orientações',
    ...
    teachingLabel: labelsConfig?.teachingLabel ?? detailSettings?.teachingLabel ?? '/// ensino',
    teachingTitle:
      labelsConfig?.teachingTitle ?? detailSettings?.teachingTitle ?? 'Disciplinas Ministradas',
    ...
    awardsLabel: labelsConfig?.awardsLabel ?? detailSettings?.awardsLabel ?? '/// prêmios',
    awardsTitle: labelsConfig?.awardsTitle ?? detailSettings?.awardsTitle ?? 'Prêmios e Distinções',
    issuerLabel: detailSettings?.issuerLabel || 'Instituição',
    institutionalLabel:
      labelsConfig?.institutionalLabel ?? detailSettings?.institutionalLabel ?? '/// posições',
    institutionalTitle:
      labelsConfig?.institutionalTitle ??
      detailSettings?.institutionalTitle ??
      'Posições Institucionais',
    collaborationsLabel:
      labelsConfig?.collaborationsLabel ??
      detailSettings?.collaborationsLabel ??
      '/// colaborações',
    collaborationsTitle:
      labelsConfig?.collaborationsTitle ??
      detailSettings?.collaborationsTitle ??
  ...
  // Styling
  const styles = {
    sectionBg: styling?.sectionBackground ?? 'bg-ufam-dark',
    headerBg: styling?.headerBackground ?? 'border-b border-white/5',
    labelColor: styling?.labelColor ?? 'text-ufam-primary',
    titleColor: styling?.titleColor ?? 'text-white',
    textColor: styling?.textColor ?? 'text-ufam-secondary',
    cardBg: styling?.cardBackground ?? 'bg-ufam-dark',
    cardBorder: styling?.cardBorder ?? 'border-white/5',
    iconColor: styling?.iconColor ?? 'text-ufam-primary',
  };

  // Visibility Helper
  const showSection = (sectionName: keyof typeof member | string, hasContent: boolean) => {
    // Check component visibility first
    if (visibility) {
      const visibilityKey =
        `show${sectionName.charAt(0).toUpperCase()}${sectionName.slice(1)}` as keyof typeof visibility;
      if (visibility[visibilityKey] === false) return false;
    }

    // Then check member specific toggle
    const memberToggleKey =
      `show${sectionName.charAt(0).toUpperCase()}${sectionName.slice(1)}` as keyof typeof member;
    const memberToggle = member[memberToggleKey];

    // Treat null/undefined as true (visible by default if has content)
    return hasContent && memberToggle !== false;
  };
  ...
  return (
    <main className="min-h-screen bg-ufam-bg pt-24">
      ...
      {/* Header Section */}
      <section className={`py-8 ${styles.headerBg}`}>
        <div className="container mx-auto px-6">
          <div className="grid md:grid-cols-3 gap-12 items-start">
            {/* Photo */}
            {visibility?.showPhoto !== false && (
            ...
            <FadeIn delay={100} className="md:col-span-2">
              ...
              {/* Name */}
              <h1 className={`text-3xl md:text-4xl font-bold ${styles.titleColor} font-tech mb-2`}>
                {member.fullName}
              </h1>

              {/* Specialization Areas */}
              {visibility?.showSpecialization !== false &&
                ...
                member.specializationAreas.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-6">
                    {member.specializationAreas.map((area: string, i: number) => (
                      <span
                        key={i}
                        className={`text-xs font-tech ${styles.textColor} bg-white/5 px-2 py-1 rounded`}
                      >
                        {area}
                      </span>
                    ))}
              ...
              {/* Short Bio */}
              {visibility?.showBio !== false && member.shortBio && (
                <p className={`${styles.textColor} leading-relaxed mb-6`}>{member.shortBio}</p>
              )}
              ...
              {visibility?.showMetrics !== false &&
                (member.hIndex || member.totalPublications || member.googleScholarCitations) && (
                  <div>
                    <h3
                      className={`font-tech ${styles.labelColor} text-xs mb-3 tracking-widest lowercase`}
                    >
                      {labels.metricsLabel}
                    </h3>
                    <div className="grid grid-cols-3 gap-4 mb-6">
                      {member.hIndex && (
                        <div
                          className={`${styles.cardBg} p-4 rounded border ${styles.cardBorder} group relative`}
                        >
                          <Award className={`w-5 h-5 ${styles.iconColor} mb-2`} />
                          <span className={`text-2xl font-bold ${styles.titleColor} font-tech`}>
                            {member.hIndex}
                          </span>
                          <p className={`text-xs ${styles.textColor} font-tech`}>h-index</p>
                          {/* Tooltip */}
                      ...
                      {member.totalPublications && (
                        <div className={`${styles.cardBg} p-4 rounded border ${styles.cardBorder}`}>
                          <BookOpen className={`w-5 h-5 ${styles.iconColor} mb-2`} />
                          <span className={`text-2xl font-bold ${styles.titleColor} font-tech`}>
                            {member.totalPublications}
                          </span>
                          <p className={`text-xs ${styles.textColor} font-tech lowercase`}>
                      ...
                      {member.googleScholarCitations && (
                        <div className={`${styles.cardBg} p-4 rounded border ${styles.cardBorder}`}>
                          <Users className={`w-5 h-5 ${styles.iconColor} mb-2`} />
                          <span className={`text-2xl font-bold ${styles.titleColor} font-tech`}>
                            {member.googleScholarCitations}
                          </span>
                          <p className={`text-xs ${styles.textColor} font-tech lowercase`}>
                  ...
                  <div className="flex flex-wrap gap-4 mb-6">
                    {member.email && (
                      <a
                        ...
                        className={`inline-flex items-center gap-2 ${styles.textColor} hover:text-ufam-primary transition-colors text-sm`}
                      >
                    ...
                    {member.phone && (
                      <span
                        className={`inline-flex items-center gap-2 ${styles.textColor} text-sm`}
                      >
              ...
              {visibility?.showSocialLinks !== false && (
                <div className="flex flex-wrap gap-3">
                  {member.googleScholarUrl && (
                    <a
                      ...
                      className={`inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded hover:border-ufam-primary/50 hover:bg-ufam-primary/10 transition-all text-sm ${styles.textColor} hover:text-white`}
                    >
      ...
      {/* Biography */}
      {showSection('biography' as any, !!member.biography) && visibility?.showBio !== false && (
        <section
          className={`py-16 ${styles.sectionBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'}`}
        >
          <div className="container mx-auto px-6">
            <FadeIn>
              <h2
                className={`font-tech ${styles.labelColor} text-sm mb-2 tracking-widest lowercase`}
              >
                {labels.bioLabel}
              </h2>
              <h3 className={`text-2xl font-bold ${styles.titleColor} font-tech mb-8`}>
                {labels.bioTitle}
              </h3>
            </FadeIn>
      ...
      {showSection(
        ...
        visibility?.showEducation !== false && (
          <section className={`py-16 ${styles.sectionBg}`}>
            <div className="container mx-auto px-6">
              <FadeIn>
                <h2
                  className={`font-tech ${styles.labelColor} text-sm mb-2 tracking-widest lowercase`}
              ...
              <div className="space-y-6">
                {member.academicFormation?.map((edu, index) => (
                  <FadeIn key={index} delay={index * 100}>
                    <div
                      className={`${styles.cardBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'} p-6 rounded border ${styles.cardBorder}`}
                    >
                      <div className="flex items-start gap-4">
                        <GraduationCap
                          className={`w-6 h-6 ${styles.iconColor} flex-shrink-0 mt-1`}
                        />
                        <div className="flex-1">
                          <h4 className={`text-lg font-bold ${styles.titleColor} mb-1`}>
                            {edu.degree}
                          </h4>
                          <p className={`${styles.textColor} mb-2`}>{edu.institution}</p>
                          <div className={`flex items-center gap-4 text-xs ${styles.textColor}`}>
                            {edu.year && <span className={styles.iconColor}>{edu.year}</span>}
      ...
      {showSection(
        ...
        visibility?.showResearchLines !== false && (
          <section
            className={`py-16 ${styles.sectionBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'}`}
          >
            <div className="container mx-auto px-6">
                ...
                {member.researchLines?.map((line, index) => (
                  <FadeIn key={line.id} delay={index * 100}>
                    <Link
                      ...
                      className={`group block ${styles.cardBg} p-6 rounded border ${styles.cardBorder} hover:border-ufam-primary/30 transition-all`}
                    >
                      <Beaker className={`w-8 h-8 ${styles.iconColor} mb-4`} />
                      <h4
                        className={`text-lg font-bold ${styles.titleColor} font-tech group-hover:text-ufam-light transition-colors mb-2`}
                      >
                        {line.title}
                      </h4>
                      <p className={`text-sm ${styles.textColor} line-clamp-2`}>
            ...
            <div className="container mx-auto px-6">
              ...
              {member.coordinatedProjects && member.coordinatedProjects.length > 0 && (
                <div className="mb-12">
                  <FadeIn>
                    <h4
                      className={`text-lg font-bold ${styles.titleColor} font-tech mb-6 flex items-center gap-2`}
                    >
                    ...
                    {member.coordinatedProjects.map((project, index) => (
                      <FadeIn key={project.id} delay={index * 100}>
                        <Link
                          ...
                          className={`block ${styles.cardBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'} p-6 rounded border border-ufam-primary/20 hover:border-ufam-primary/50 transition-all group`}
                        >
                          <h5
                            className={`${styles.titleColor} font-bold mb-2 group-hover:text-ufam-light transition-colors`}
                          >
                            {project.title}
                          </h5>
                          <p className={`text-sm ${styles.textColor} line-clamp-2`}>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    {member.participatingProjects.map((project, index) => (
                      <FadeIn key={project.id} delay={index * 100}>
                        <Link
                          href={`/projects/${project.slug}`}
                          className={`block ${styles.cardBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'} p-6 rounded border ${styles.cardBorder} hover:border-ufam-primary/30 transition-all group`}
                        >
                          <h5
                            className={`${styles.titleColor} font-bold mb-2 group-hover:text-ufam-light transition-colors`}
                          >
      ...
      {showSection(
        ...
        visibility?.showPublications !== false && (
          <section
            className={`py-16 ${styles.sectionBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'}`}
          >
            <div className="container mx-auto px-6">
              ...
              {member.publications && member.publications.length > 0 ? (
                    ...
                    {member.publications.slice(0, 10).map((pub, index) => (
                      <FadeIn key={pub.id} delay={index * 50}>
                        <Link
                          ...
                          className={`block ${styles.cardBg} p-6 rounded border ${styles.cardBorder} hover:border-ufam-primary/30 transition-all group`}
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <h4
                                className={`${styles.titleColor} font-medium group-hover:text-ufam-light transition-colors mb-2`}
                              >
                                {pub.title}
                              </h4>
                              <p className={`text-sm ${styles.textColor}`}>{pub.authorsText}</p>
                              <div
                                className={`flex items-center gap-4 mt-2 text-xs ${styles.textColor}`}
                              >
                                <span className={styles.iconColor}>{pub.year}</span>
                            ...
                            {pub.citationCount && pub.citationCount > 0 && (
                              <div className="text-right">
                                <span className={`text-lg font-bold ${styles.iconColor} font-tech`}>
                                  {pub.citationCount}
                                </span>
                                <p className={`text-xs ${styles.textColor} lowercase`}>
                                  {labels.citationsLabel}
                                </p>
                              </div>
                            )}
                  ...
                  {member.publications.length > 10 && (
                    <FadeIn delay={500}>
                      <div className="text-center mt-8">
                        <Link
                          href="/publications"
                          className={`inline-flex items-center gap-2 ${styles.iconColor} hover:text-ufam-light transition-colors font-tech text-sm`}
                        >
                ...
                <p className={`${styles.textColor} text-center py-8`}>
                ...
                <div className="mb-12">
                  <FadeIn>
                    <h4
                      className={`text-lg font-bold ${styles.titleColor} font-tech mb-6 flex items-center gap-2`}
                    >
                      <UserCheck className={`w-5 h-5 ${styles.iconColor}`} />
                    ...
                    {member.currentGraduateAdvisees.map((advisee, index) => (
                      <FadeIn key={index} delay={index * 100}>
                        <div
                          className={`${styles.cardBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'} p-6 rounded border ${styles.cardBorder}`}
                        >
                          <h5 className={`${styles.titleColor} font-bold mb-1`}>{advisee.name}</h5>
                          <p className={`text-sm ${styles.textColor} mb-3`}>
                            {advisee.topic || 'Sem tema definido'}
                          </p>
                          <div className={`flex items-center gap-4 text-xs ${styles.textColor}`}>
                            <span className={styles.iconColor}>{advisee.level}</span>
                            {advisee.year && (
                              <span>
                    ...
                    {member.completedAdvisees.map((advisee, index) => (
                      <FadeIn key={index} delay={index * 100}>
                        ...
                        >
                          ...
                          <div className={`flex flex-col gap-2 text-xs ${styles.textColor}`}>
      ...
      {/* Teaching */}
      {showSection('teaching' as any, !!(member.teachingGraduate || member.teachingPostgraduate)) &&
        visibility?.showTeaching !== false && (
          <section
            className={`py-16 ${styles.sectionBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'}`}
          >
            <div className="container mx-auto px-6">
                ...
                {member.teachingGraduate &&
                  ...
                  member.teachingGraduate.length > 0 && (
                    <FadeIn delay={100}>
                      <div>
                        <h4
                          className={`text-lg font-bold ${styles.titleColor} font-tech mb-4 flex items-center gap-2`}
                        >
                        ...
                        <ul className="space-y-3">
                          {member.teachingGraduate.map((course: string, index: number) => (
                            <li
                              key={index}
                              className={`${styles.textColor} pl-4 border-l-2 border-ufam-primary/30`}
                            >
                              {course}
                            </li>
                          ))}
              ...
              <div className="space-y-6">
                {member.awardsDistinctions?.map((award, index) => (
                  <FadeIn key={index} delay={index * 100}>
                    ...
                    >
                      <div className="flex items-start gap-4">
                        <Trophy className={`w-6 h-6 ${styles.iconColor} flex-shrink-0 mt-1`} />
                        <div className="flex-1">
                          <h4 className={`text-lg font-bold ${styles.titleColor} mb-1`}>
                            {award.title}
                          </h4>
                          ...
                          {award.year && (
                            <span className={`text-xs ${styles.iconColor}`}>{award.year}</span>
                          )}
                          {award.description && (
                            <p className={`text-xs ${styles.textColor} mt-1 italic`}>
                              {award.description}
                            </p>
                          )}
                        </div>
      ...
      {showSection(
        ...
        visibility?.showInstitutionalPositions !== false && (
          <section
            className={`py-16 ${styles.sectionBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'}`}
          >
            <div className="container mx-auto px-6">
              ...
              <div className="space-y-4">
                {member.institutionalPositions?.map((position: string, index: number) => (
                  <FadeIn key={index} delay={index * 100}>
                    <div className={`${styles.cardBg} p-6 rounded border ${styles.cardBorder}`}>
                      <div className="flex items-start gap-4">
                        <Briefcase className={`w-6 h-6 ${styles.iconColor} flex-shrink-0 mt-1`} />
                        <p className={`flex-1 ${styles.textColor}`}>{position}</p>
                      </div>
                    </div>
                  </FadeIn>
                ))}
      ...
      {showSection('collaborations' as any, !!member.internationalCollaborations) &&
        visibility?.showCollaborations !== false && (
          <section className={`py-16 ${styles.sectionBg}`}>
            <div className="container mx-auto px-6">
              <FadeIn>
                <h2
              ...
              <FadeIn delay={100}>
                <div
                  className={`${styles.cardBg === 'bg-ufam-dark' ? 'bg-ufam-bg' : 'bg-ufam-dark'} p-8 rounded border ${styles.cardBorder}`}
                >
